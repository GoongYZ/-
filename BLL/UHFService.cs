...
namespace BLL
{
    ...
    public class UHFService
    {
        ...
        public static void ConnectCOM()
        {
            ...
            if (reader != null)
                Logo.sWriteLogo("读写器" + reader.m_rfidWorkReader.m_strDeviceID + ",序列号为" + reader.m_rfidWorkReader.m_strDeviceID + "已连接", 7);
            else
            {
                ...
                if (nRetVal == OperationResult.SUCCESS)
                {
                    ...
                    Logo.sWriteLogo("读写器" + m_rfidClientReader.m_strDeviceID + ",序列号为" + m_rfidClientReader.m_strDeviceID + "已连接", 7);
                }
                else
                {
                    Logo.sWriteLogo("读写器连接失败：原因" + nRetVal, 7);
                }
            }
        }
        ...
        private static void m_rifdServer_m_OnErrorOccured(object sender, ErrorReportEventArgs e)
        {
            if (e != null)
            {
                ...
                {
                    Logo.sWriteLogo("有相同设备序列号 + e.m_strDeviceID + 的读写器 + e.m_strMacError + 连接到本地服务器", 7);
                }
                else
                {
                    Logo.sWriteLogo("读写器 + e.m_strDeviceID + 发生错误,ErrorCode: + e.m_strErrorCode + ,MacCode: + e.m_strMacError", 7);
                }
            }
        }
        /// <summary>
        ...
        private static void m_rifdServer_m_OnUnregistered(object sender, UnregisteredEventArgs e)
        {
            if (e != null)
            {
                ...
                if (null != operateReader)
                {
                    ...
                    Logo.sWriteLogo("读写器" + e.m_strDeviceID + "从本地服务器中断开", 7);
        ...
        private static void m_rfidClientReader_m_OnInventoryReport(object sender, InventoryReportEventArgs e)
        {
            if (e != null)
            {
                ...
                Logo.sWriteLogo(string.Format("读写器:{0},EPC:{1},TID:{2},天线号:{3},RSSI:{4},频点:{5},盘点时刻:{6}", report.m_strDeviceID, report.m_strEPC, report.m_strTID, report.m_strAntennaNo, report.m_strRSSI, report.m_strFrequency, report.m_strTimeStamp), 7);
        ...
        private static void m_rfidClientReader_m_OnDisconnect(object sender, DisconnectEventArgs e)
        {
            if (e != null)
            {
                Logo.sWriteLogo(string.Format("读写器:{0}断开连接", e.m_strDeviceID), 7);
                Reader operateReader = ReaderManager.g_ReaderManager.GetRFIDReaderByDeviceID(e.m_strDeviceID);
                if (null != operateReader)
                {
                    m_strLostConnectDeviceID = e.m_strDeviceID;
